...
function convertDataFilesToJSON(dataFiles) {
    let promises = _.map(dataFiles, filePath => {
        const pathObject = path.parse(filePath);
        const relDir = path.relative(process.cwd(), pathObject.dir);
        const ext = pathObject.ext.substring(1);
        if (!_.has(supportedExtensions, ext)) {
            return null;
        }
        return fse.readFile(filePath).then(data => {
            const propPath = relDir.split(path.sep).concat(pathObject.name);
...
exports.sourceNodes = (props, pluginOptions = {}) => {
    ...
    return readDirRecursively(pluginOptions.path).then(dataFiles => {
        ...
        return convertDataFilesToJSON(sortedDataFiles);
    }).then(data => {
        actions.createNode({
            id: 'SiteData',
            parent: null,
